...
assert.match(main, /const maxMountAttempts = 12;/);
assert.match(main, /if \(mountAttempt < maxMountAttempts\) schedule\(render\)/);
assert.match(main, /this\.ensureSelectionDismissalHandlers\(element\.ownerDocument\)/);
assert.match(main, /target\.closest\("\.va-annotation, \.va-rail-item"\)/);
assert.match(main, /event\.key === "Escape" \|\| event\.key === "Esc"/);
